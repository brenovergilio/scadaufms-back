...
export default class UpdateSpecificMedidorMD30 extends BaseMedidorMD30UseCases {
  async execute(input: InputUpdateSpecificMedidorMD30): Promise<MedidorMD30> {
    ...
    const medidorMD30 = await this.medidorMD30Repository.getMedidorMD30ByID(
      input.medidorID
    );
    ...
    if (input.rushMinute !== undefined)
      medidorMD30.rush.minute = input.rushMinute;
    if (input.rushInterval !== undefined)
      medidorMD30.rush.interval = input.rushInterval;
