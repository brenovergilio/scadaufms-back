...
export default class GetFatoresDePotenciaCorrigidosUseCase extends BaseMedicaoMD30UseCases {
  ...
  async execute(id: string, potenciaAparenteCapacitiva: number, dateRange: DateRange): Promise<Array<MedicaoMD30>> {
    if (Number.isNaN(Number(potenciaAparenteCapacitiva))) throw new InvalidValueError();
    ...
    const potenciasCorrigidas = potencias.map((potencias) => {
      ...
      const fatorPotenciaCorrigido = potenciaAtiva / (Math.sqrt((Math.pow((potenciaAparente - potenciaAparenteCapacitiva), 2) + Math.pow(potenciaAtiva, 2))));
      const mapValue = new Map<string, number>();
      mapValue.set('Fator de Potencia Corrigido', fatorPotenciaCorrigido);
      return new MedicaoMD30(potencias.measurerID, potencias.timestamp, mapValue);
    });
